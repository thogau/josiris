...
public class ImportView extends VerticalLayout {
	...
	public ImportView(PatientService service, AutowireCapableBeanFactory beanFactory) {
		this.service = service;
		this.beanFactory = beanFactory;

		upload.addAllFinishedListener(event -> {
			LoggerFactory.getLogger(ImportView.class.getName()).warn("Processing pivot files...");
			try {
				populateDatabase();
				LoggerFactory.getLogger(ImportView.class.getName()).warn("Processing pivot files done.");
			} catch (Exception e) {
