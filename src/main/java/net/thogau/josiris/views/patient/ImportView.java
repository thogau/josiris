...
public class ImportView extends VerticalLayout {
	...
	public ImportView(PatientService service, ConceptualDomainService cdService,
			AutowireCapableBeanFactory beanFactory) {
		...
		upload.addAllFinishedListener(event -> {
			LoggerFactory.getLogger(getClass().getName()).info("Processing pivot files...");
			try {
				populateDatabase();
				LoggerFactory.getLogger(getClass().getName()).info("Processing pivot files done.");
				UI.getCurrent().getPage().setLocation("patients");
			} catch (Exception e) {
				e.printStackTrace();
				Notification.show("Patients could not be imported", 5000, Position.TOP_CENTER).addThemeName("error");
			}
		});
	...
	private void populateDatabase() {
		...
		try {
			patients = new CsvToBeanBuilder<Patient>(
					new InputStreamReader(buffer.getInputStream("OSIRIS_pivot_Patient.csv"))).withType(Patient.class)
					.withMappingStrategy(patientStrategy).build().parse();
		} catch (Exception e) {
			Notification.show("Error parsing OSIRIS_pivot_Patient.csv : " + e.getMessage(), 5000, Position.TOP_CENTER)
					.addThemeName("error");
		...
		try {
			...
			Notification.show("Error parsing OSIRIS_pivot_Treatment.csv : " + e.getMessage(), 5000, Position.TOP_CENTER)
					.addThemeName("error");
			throw new RuntimeException();
		}
		try {
			drugs = new CsvToBeanBuilder<CsvDrug>(new InputStreamReader(buffer.getInputStream("OSIRIS_pivot_Drug.csv")))
					.withType(CsvDrug.class).withMappingStrategy(drugStrategy).build().parse();
		} catch (Exception e) {
		...
		try {
			analysises = new CsvToBeanBuilder<Analysis>(
					new InputStreamReader(buffer.getInputStream("OSIRIS_pivot_Analysis.csv"))).withType(Analysis.class)
					.withMappingStrategy(anaStrategy).build().parse();
		} catch (Exception e) {
			Notification.show("Error parsing OSIRIS_pivot_Analysis.csv : " + e.getMessage(), 5000, Position.TOP_CENTER)
					.addThemeName("error");
			throw new RuntimeException();
		}
		try {
			...
			Notification.show("Error parsing OSIRIS_pivot_Fusion.csv : " + e.getMessage(), 5000, Position.TOP_CENTER)
					.addThemeName("error");
			throw new RuntimeException();
		}
		try {
			...
			Notification
					.show("Error parsing OSIRIS_pivot_Annotation.csv : " + e.getMessage(), 5000, Position.TOP_CENTER)
					.addThemeName("error");
			throw new RuntimeException();
		}
		...
		for (Patient patient : patientIds.values()) {
			try {
				...
				Notification.show("Error importing patient " + patient.getOriginalId() + " : " + e.getMessage(), 5000,
						Position.TOP_CENTER).addThemeName("error");
				e.printStackTrace();
				throw new RuntimeException();
			}
